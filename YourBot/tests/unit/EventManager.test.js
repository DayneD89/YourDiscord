...
// Mock timers to control setInterval/setTimeout
jest.useFakeTimers();
...
describe('EventManager', () => {
    ...
    describe('constructor', () => {
        ...
        test('should start reminder checker', () => {
            // Check that setInterval was called for reminder checker
        ...
        test('should schedule initial reminder check', () => {
            // Check that setTimeout was called for initial check
    ...
    describe('checkReminders', () => {
        test('should process upcoming events', async () => {
            ...
            const processEventReminderSpy = jest.spyOn(eventManager, 'processEventReminder')
                .mockResolvedValue();

            await eventManager.checkReminders();

            expect(mockEventStorage.getUpcomingEvents).toHaveBeenCalledWith('guild123');
            expect(processEventReminderSpy).toHaveBeenCalledWith(upcomingEvents[0]);
        });
        ...
        test('should handle errors gracefully', async () => {
            ...
            expect(consoleSpy).toHaveBeenCalledWith('Error checking reminders:', expect.any(Error));
    ...
    describe('startReminderChecker - development mode', () => {
        test('should use dev mode intervals when weekReminder is negative', () => {
            const consoleSpy = jest.spyOn(console, 'log').mockImplementation();
            ...
            const devMockBot = {
                ...mockBot,
                getReminderIntervals: jest.fn().mockReturnValue({
                    weekReminder: -2 * 60 * 1000, // Negative means dev mode
                    dayReminder: -1 * 60 * 1000
                })
            };
            
            // Create new EventManager to trigger constructor
            new EventManager(devMockBot);

            expect(consoleSpy).toHaveBeenCalledWith(
                expect.stringContaining('checking every 30s (dev mode)')
            );
            expect(mockSetInterval).toHaveBeenCalledWith(
                expect.any(Function),
                30 * 1000 // 30 seconds in dev mode
            );
            expect(mockSetTimeout).toHaveBeenCalledWith(
                expect.any(Function),
                10000 // 10 seconds in dev mode
            );

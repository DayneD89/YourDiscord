...
class BotLifecycleManager {
    ...
    async postDeploymentConfirmation() {
        try {
            ...
            console.log(`🔍 Guild ID: ${this.bot.getGuildId()}`);
            console.log(`🔍 Command Channel ID: ${this.bot.getCommandChannelId()}`);
            
            const guild = this.client.guilds.cache.get(this.bot.getGuildId());
            if (!guild) {
                console.error(`❌ Guild not found for deployment confirmation. Guild ID: ${this.bot.getGuildId()}`);
            ...
            if (!modBotChannel) {
                console.error(`❌ Moderator bot channel not found. Channel ID: ${this.bot.getCommandChannelId()}`);
            ...
            const confirmationMessage = `🤖 **Bot ${this.bot.getRunId()} Online** - New version deployed and ready\n⏰ ${timestamp}\n⚡ Startup: ${startupTime}s`;
    ...
    async sendShutdownMessage(reason) {
        ...
        const guild = this.client.guilds.cache.get(this.bot.getGuildId());
        if (!guild) {
            console.log(`❌ Guild not found for shutdown message. Guild ID: ${this.bot.getGuildId()}`);
            return;
        }
        console.log(`✅ Found guild: ${guild.name} (${guild.id})`);
        ...
        const modBotChannel = guild.channels.cache.get(this.bot.getCommandChannelId());
        if (!modBotChannel) {
            console.log(`❌ Moderator bot channel ${this.bot.getCommandChannelId()} not found`);
            console.log(`❌ Available channels: ${Array.from(guild.channels.cache.values()).map(ch => `${ch.name}(${ch.id})`).join(', ')}`);
            return;
        }
        ...
        const shutdownMessage = `🔄 **Bot ${this.bot.getRunId()} Shutting Down** - ${reason}\n⏰ ${timestamp}\n⚡ Uptime: ${uptime}s`;
